...
const PLATFORMS = {
  bitcoin: [
    ...
    { name: 'Compound', apr: 5.1,  ltv: 65, liq: 80,   type: 'DeFi', color: '#00D395', link: 'https://compound.finance', best: false, founded: '2018', country: 'Décentralisé',  users: '200K+', regulated: false, about: 'Protocole DeFi pionnier. Gouvernance décentralisée via token COMP. Taux variables.' },
  ],
  ethereum: [
    ...
    { name: 'Compound', apr: 4.5,  ltv: 75, liq: 80,   type: 'DeFi', color: '#00D395', link: 'https://compound.finance', best: false, founded: '2018', country: 'Décentralisé', users: '200K+', regulated: false, about: 'Protocole DeFi pionnier. Taux variables selon offre et demande. Gouvernance COMP.' },
    { name: 'Spark',    apr: 4.1,  ltv: 74, liq: 79,   type: 'DeFi', color: '#FF6B35', link: 'https://spark.fi',         best: false, founded: '2023', country: 'Décentralisé', users: '50K+',  regulated: false, about: 'Protocole DeFi basé sur MakerDAO. Taux stables compétitifs. Spécialisé DAI/USDS.' },
  ],
}
  ...
  return (
    <>
      ...
      <main style={{ background: '#fff', minHeight: '100vh', paddingTop: '52px' }}>
        ...
        {/* ── TOOLBAR ── */}
        <div style={{ borderBottom: '1px solid #F0F0F0', borderTop: '1px solid #F0F0F0', marginTop: '16px', background: '#FAFAFA' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              {CRYPTOS.map(x => (
                <button key={x.id} onClick={() => { setCrypto(x.id); setAmount(1); setFilter('all') }} style={{
                  ...
                  {x.name} <span style={{ fontSize: '11px', opacity: 0.4 }}>{x.symbol}</span>
                </button>
              ))}
            </div>
              ...
              <div key={p.name} style={{ borderBottom: i < rows.length - 1 ? '1px solid #F7F7F7' : 'none' }}>
                ...
                onMouseLeave={e => e.currentTarget.style.background = p.best ? '#FAFAFA' : '#fff'}>
                  ...
                  <div>
                    ...
                    <div style={{ height: '2px', background: '#F0F0F0', borderRadius: '2px', marginTop: '6px', width: '52px' }}>
                  ...
                  <a href={p.link} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} style={{
                    ...
                    border: `1.5px solid ${p.best ? '#111' : '#E0E0E0'}`,
                  }}
              ...
              {[
                ...
                : (
                  <div key={ci} style={{ padding: '20px 24px' }}>
                    ...
                    {col.rows.map(r => (
                      <div key={r[0]} style={{ display: 'flex', justifyContent: 'space-between', padding: '9px 0', borderBottom: '1px solid #F7F7F7', fontSize: '13px' }}>
